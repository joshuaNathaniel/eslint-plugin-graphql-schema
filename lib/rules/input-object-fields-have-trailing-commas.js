module.exports = {
    ...
    create: (context) => {
        return {
            InputObjectTypeDefinition(inputObjectTypeNode) {
                inputObjectTypeNode.fields.forEach(node => {
                    const line = node.loc.source.body.substring(node.oldLoc.start, node.oldLoc.end + 1);
                    const fieldName = node.name.value;
                    const parentName = inputObjectTypeNode.name.value;
                    ...
                    switch(context.options[0]) {
                        ...
                        case 'always':
                            ...
                            break;
                        case 'multiline':
                            if (inputObjectTypeNode.fields[inputObjectTypeNode.fields.length - 1] === node) {
                                if (hasTrailingComma) {
                                    context.report({
                                        node: node,
                                        message: `The field \`${parentName}.${fieldName}\` has a trailing comma.`,
